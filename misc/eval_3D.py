...
@torch.no_grad()
def compute_level_grid(opt, var, points_3D, N=None):
    ...
    for i in range(0, N, slice_batch_size):
        ...
        level_batch = var.impl_func(var, points_3D_batch)  # [B,N',N,N,1]
...
@torch.no_grad()
def compute_deformation_field(opt,var,points_3D,N=None):
    # process points in batches
    all_pts_canonical = []
    all_pts_deformations = []
    N = points_3D.shape[1]
    slice_batch_size = 10000
    for i in range(0,N,slice_batch_size):
        points_3D_batch = points_3D[:,i:i+slice_batch_size] # [B,N',N,N,3]
        _, pts_deformations = var.impl_func(var,points_3D_batch, get_deformations=True) # [B,N',N,N,1]
        pts_canonical = points_3D_batch + pts_deformations
        all_pts_canonical.append(pts_canonical)
        all_pts_deformations.append(pts_deformations)
    all_pts_canonical = torch.cat(all_pts_canonical,dim=1) # [B,N,N,N]
    all_pts_deformations = torch.cat(all_pts_deformations,dim=1) # [B,N,N,N]
    return all_pts_canonical, all_pts_deformations

@torch.no_grad()
def generate_3D(opt, var):
    points_3D = get_dense_3D_grid(opt, var)  # [B,N,N,N,3]
    level_vox = compute_level_grid(opt, var, points_3D=points_3D)  # [B,N,N,N,K]
    *level_grids, = level_vox.cpu().numpy()  # numpy equivalent to torch.unbind
    ...
    if opt.evaluation and opt.is_stage_two_active:
        var.dpc_pred_canonical, var.dpc_pred_deformations = compute_deformation_field(opt, var, var.dpc_pred)


@torch.no_grad()
def compute_chamfer_dist(opt, var):
    generate_3D(opt, var)
    if "dpc" not in var.keys():
        return 0, 0
    if opt.eval.icp:
        var.dpc_pred = ICP(opt, var.dpc_pred, var.dpc.points)
    dist_acc, dist_comp, _, _ = chamfer_distance(
        opt, X1=var.dpc_pred, X2=var.dpc.points)
...
def convert_to_explicit_worker(opt, i, level_vox_i, isoval, meshes, pointclouds=None):
    ...
    if pointclouds is not None:
        ...
        if len(mesh.triangles) != 0:
            ...
            points = np.zeros([opt.eval.num_points, 3])
