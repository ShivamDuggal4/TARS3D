...
def tb_image(opt, tb, step, group, name, images, masks=None, num_vis=None, from_range=(0, 1), poses=None, cmap="gray"):
    ...
    if poses is not None:
        ...
        images = torch.stack([draw_pose(opt, image, rot, size=20, width=2)
                             for image, rot in zip(images, rots)], dim=0)
...
def dump_point_cloud(opt, idx, name, point_clouds):
    for i, point_cloud in zip(idx, point_clouds):
        fname = "{}/dump/{}_{}.npy".format(opt.output_path, i, name)
...
def vis_pointcloud(opt, vis, step, split, pred, GT=None):
    ...
    for i in range(opt.visdom.num_samples):
        if GT is not None:
            ...
            ) for points, color in zip([pred], ["blue"])]
